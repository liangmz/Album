...
public class DBCreateDelete : IDisposable
{
    ...
    public int InsertPicture2(String[] PNAME, int AID, String[] FILEMD5, string[] PATH,int POWER)
    {
        try
        {
            int COUNT = 0;
    ...
    /// <returns>-2:用户名存在 / -3:邮箱存在 / -4:其他错误</returns>
    public int InsertUser(String USERNAME, String PASSWORD, String EMAIL, String VCODE)
    {
        object U = _ExecuteSQL_One("select u_id from a_user where u_userName = " + USERNAME);
        int UID = Convert.ToInt32(U == null || (Int32)U!=0 ? U : -2);
        if (UID == -2) return UID;

        object E = _ExecuteSQL_One("select u_id from a_user where u_Email = " + EMAIL);
        int EID = Convert.ToInt32(E == null || (Int32)E != 0 ? E : -3);
        if (EID == -3) return EID;
    ...
    private int _ExecuteSQL_Bat(String[] SQL)
    {
        int COUNT = 0;
        if (CON.State != ConnectionState.Open) CON.Open();
        COM.Connection = CON;
